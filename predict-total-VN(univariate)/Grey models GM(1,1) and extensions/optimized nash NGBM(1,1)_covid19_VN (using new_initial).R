...
for (i in 1:length(range_n)) {
  for (j in 1:length(range_P)) {
    ...
    covid19_VN_hat_1 <- NULL
    for (p in 1:m) {
      covid19_VN_hat_1[p] = ((AGO[m]^(1 - range_n[i]) - b/a) * exp(-a * (1 - range_n[i]) * (p - m)) + b/a) ^ (1/(1 - range_n[i]))
    }
...
E = matrix(nrow = 1, ncol = m)
A = matrix(nrow = 1, ncol = m)
for (k in 1:m){
  E[k] = exp(-a * (1-n_optimal) * (k-m))
  A[k] = AGO[k]^(1-n_optimal) - b/a * (1 - E[k])
}
new_optimal = sum(A*E) / sum(E^2)

for (k in 1: length(covid19_VN)) {
  covid19_VN_hat_1[k] = ((new_optimal - b/a) * exp(-a * (1 - n_optimal) * (k - m)) + b/a) ^ (1/(1 - n_optimal))
}
...
print(n_optimal)
print(P_optimal)
print(new_optimal - AGO[m]^(1 - n_optimal))
print(covid19_VN_hat_0)
print(RPE)
print(ARPE)

for (k in 1: (length(covid19_VN) +14)) {
  covid19_VN_predict_1[k] = ((new_optimal - b/a) * exp(-a * (1 - n_optimal) * (k - m)) + b/a) ^ (1/(1 - n_optimal))
}
