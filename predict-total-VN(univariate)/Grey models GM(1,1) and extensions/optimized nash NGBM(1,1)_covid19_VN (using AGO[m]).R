...
for (i in 1:length(range_n)) {
  for (j in 1:length(range_P)) {
    ...
    covid19_VN_hat_1 <- NULL
    for (l in 1:m) {
      covid19_VN_hat_1[l] = ((AGO[m]^(1 - range_n[i]) - b/a) * exp(-a * (1 - range_n[i]) * (l - m)) + b/a) ^ (1/(1 - range_n[i]))
    }
...
for (k in 1: length(covid19_VN)) {
  covid19_VN_hat_1[k] = ((AGO[m]^(1 - n_optimal) - b/a) * exp(-a * (1 - n_optimal) * (k - m)) + b/a) ^ (1/(1 - n_optimal))
}
...
for (k in 1: (length(covid19_VN) + 14)) {
  covid19_VN_predict_1[k] = ((AGO[m]^(1 - n_optimal) - b/a) * exp(-a * (1 - n_optimal) * (k - m)) + b/a) ^ (1/(1 - n_optimal))
}
